...
from book b,
    (select bookid from orders where custid =
        (select custid from customer where name = '박지성')) o
where b.bookid = o.bookid;

-- (6) 박지성이구매한도서의이름, 가격, 정가와판매가격의차이
select b.bookname, (b.price-o.saleprice) as "차액"
...
-- (9) 주문금액의총액과주문의평균금액
select sum(saleprice) as "TOTAL", round(avg(saleprice)) as "AVGSALEPRICE"
from orders;
...
from customer c natural join orders natural join book b
...
where (b.price-o.saleprice) =
    (select max(b.price-o.saleprice) from orders o natural join book b);
...
select c.name
from customer c natural join book b natural join orders
...
select c.name
from customer c natural join book b natural join orders o
group by c.name
